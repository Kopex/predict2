 ...
 */
class Predict2_Time extends Predict2_Math
{
    ...
    public static function ThetaG($epoch, Predict2_Model_DeepArg $deep_arg)
    {
        ...
        $GMST = Predict2_Math::Modulus($GMST + predict2::secday * predict2::omega_E * $UT, predict2::secday);
        $deep_arg->ds50 = $jd - 2433281.5 + $UT;

        return Predict2_Math::FMod2p(6.3003880987 * $deep_arg->ds50 + 1.72944494);
    }
    ...
    {
        /* Reference:  The 1992 Astronomical Almanac, page B6. */
        $UT   = Predict2_Math::Frac($jd + 0.5);
        $jd   = $jd - $UT;
        $TU   = ($jd - 2451545.0) / 36525;
        $GMST = 24110.54841 + $TU * (8640184.812866 + $TU * (0.093104 - $TU * 6.2E-6));
        $GMST = Predict2_Math::Modulus($GMST + predict2::secday * predict2::omega_E * $UT, predict2::secday);

        return predict2::twopi * $GMST / predict2::secday;
    }

    /**
     * Read the system clock and return the current Julian day.  From phpPredict2
     *
     * @return float
     */
    ...
    public static function Delta_ET($year)
    {
      ...
      $delta_et = 26.465 + 0.747622 * ($year - 1950) +
                 1.886913 * sin(predict2::twopi * ($year - 1975) / 33);

      return $delta_et;
    }

    /**
     * Converts a daynum to a unix timestamp.  From phpPredict2.
     *
    ...
    public static function daynum2unix($dn) {
        ...
        return (int)round(86400.0 * ($dn - 2444238.5 + 3651.0));
    }
     ...
     *
     * @param Predict2_TLE $tle The TLE object
     *
     * @return int
     */
    public static function getEpochTimeStamp(Predict2_Model_TLE $tle)
    {
        $year = $tle->epoch_year;
        $day  = $tle->epoch_day;
