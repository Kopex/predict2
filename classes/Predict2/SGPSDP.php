 ...
 */
class Predict2_SGPSDP extends Predict2_Time
{
    const ALL_FLAGS             = -1;
    ...
    public function SGP4(Predict2_Sat $sat, $tsince)
    {
        /* Initialization */
        if (~$sat->flags & self::SGP4_INITIALIZED_FLAG) {
            ...
            $a1 = pow(predict2::xke / $sat->tle->xno, predict2::tothrd);
            ...
            $betao2 = 1 - $eosq;
            $betao = sqrt($betao2);
            $del1 = 1.5 * predict2::ck2 * $sat->sgps->x3thm1 / ($a1 * $a1 * $betao * $betao2);
            $ao = $a1 * (1 - $del1 * (0.5 * predict2::tothrd + $del1 * (1 + 134.0 / 81.0 * $del1)));
            $delo = 1.5 * predict2::ck2 * $sat->sgps->x3thm1 / ($ao * $ao * $betao * $betao2);
            ...
            if (($sat->sgps->aodp * (1.0 - $sat->tle->eo) / predict2::ae) < (220.0 / predict2::xkmper + predict2::ae)) {
            ...
            $s4 = predict2::__s__;
            $qoms24 = predict2::qoms2t;
            $perige = ($sat->sgps->aodp * (1 - $sat->tle->eo) - predict2::ae) * predict2::xkmper;
            if ($perige < 156.0) {
                if ($perige <= 98.0) {
                    $s4 = 20.0;
                } else {
                ...
                $qoms24 = pow((120.0 - $s4) * predict2::ae / predict2::xkmper, 4);
                $s4 = $s4 / predict2::xkmper + predict2::ae;
            ...
            $c2 = $coef1 * $sat->sgps->xnodp * ($sat->sgps->aodp *
                            ...
                            0.75 * predict2::ck2 * $tsi / $psisq * $sat->sgps->x3thm1 *
                            (8.0 + 3.0 * $etasq * (8 + $etasq)));
            $sat->sgps->c1 = $c2 * $sat->tle->bstar;
            $sat->sgps->sinio = sin($sat->tle->xincl);
            $a3ovk2 = -predict2::xj3 / predict2::ck2 * pow(predict2::ae, 3);
            $c3 = $coef * $tsi * $a3ovk2 * $sat->sgps->xnodp * predict2::ae * $sat->sgps->sinio / $sat->tle->eo;
            ...
            $sat->sgps->c4 = 2.0 * $sat->sgps->xnodp * $coef1 * $sat->sgps->aodp * $betao2 *
                ($sat->sgps->eta * (2.0 + 0.5 * $etasq) +
                 ...
                 2.0 * predict2::ck2 * $tsi / ($sat->sgps->aodp * $psisq) *
            ...
            $temp1 = 3.0 * predict2::ck2 * $pinvsq * $sat->sgps->xnodp;
            $temp2 = $temp1 * predict2::ck2 * $pinvsq;
            $temp3 = 1.25 * predict2::ck4 * $pinvsq * $pinvsq * $sat->sgps->xnodp;
            ...
            $sat->sgps->xmcof = -predict2::tothrd * $coef * $sat->tle->bstar * predict2::ae / $eeta;
        ...
        $capu = Predict2_Math::FMod2p($xlt - $xnode);
        $temp2 = $capu;
        ...
        $r = $a * (1.0 - $ecose);
        $temp1 = 1.0 / $r;
        $rdot = predict2::xke * sqrt($a) * $esine * $temp1;
        $rfdot = predict2::xke * sqrt($pl) * $temp1;
        $temp2 = $a * $temp1;
        $betal = sqrt($temp);
        $temp3 = 1.0 / (1.0 + $betal);
        $cosu = $temp2 * ($cosepw - $axn + $ayn * $esine * $temp3);
        $sinu = $temp2 * ($sinepw - $ayn - $axn * $esine * $temp3);
        $u = Predict2_Math::AcTan($sinu, $cosu);
        $sin2u = 2.0 * $sinu * $cosu;
        $cos2u = 2.0 * $cosu * $cosu - 1.0;
        $temp = 1.0 / $pl;
        $temp1 = predict2::ck2 * $temp;
        $temp2 = $temp1 * $temp;
        ...
        $sat->phase = $xlt - $xnode - $omgadf + predict2::twopi;
        if ($sat->phase < 0) {
            $sat->phase += predict2::twopi;
        }
        $sat->phase = Predict2_Math::FMod2p($sat->phase);
    ...
    /* are vector_t structures returning ECI satellite position and */
    /* velocity. Use Convert_Sat_State() to convert to km and km/s. */
    public function SDP4(Predict2_Sat $sat, $tsince)
    {
        /* Initialization */
        if (~$sat->flags & self::SDP4_INITIALIZED_FLAG) {
            ...
            $del1 = 1.5 * predict2::ck2 * $sat->sgps->x3thm1 /
                ($a1 * $a1 * $sat->deep_arg->betao * $sat->deep_arg->betao2);
            $ao = $a1 * (1.0 - $del1 * (0.5 * predict2::tothrd + $del1 * (1.0 + 134.0 / 81.0 * $del1)));
            $delo = 1.5 * predict2::ck2 * $sat->sgps->x3thm1 /
            ...
            $s4 = predict2::__s__;
            $qoms24 = predict2::qoms2t;
            $perige = ($sat->deep_arg->aodp * (1.0 - $sat->tle->eo) - predict2::ae) * predict2::xkmper;
            if ($perige < 156.0) {
                if ($perige <= 98.0) {
                    $s4 = 20.0;
                } else {
            ...
            $c2 = $coef1 * $sat->deep_arg->xnodp * ($sat->deep_arg->aodp *
                                (1.0 + 1.5 * $etasq + $eeta *
                                 (4.0 + $etasq)) + 0.75 * predict2::ck2 * $tsi / $psisq *
            ...
            $sat->sgps->c4 = 2.0 * $sat->deep_arg->xnodp * $coef1 *
                ...
                ($eta * (2.0 + 0.5 * $etasq) + $sat->tle->eo *
                 (0.5 + 2.0 * $etasq) - 2.0 * predict2::ck2 * $tsi /
            ...
            $temp1 = 3.0 * predict2::ck2 * $pinvsq * $sat->deep_arg->xnodp;
            $temp2 = $temp1 * predict2::ck2 * $pinvsq;
            $temp3 = 1.25 * predict2::ck4 * $pinvsq * $pinvsq * $sat->deep_arg->xnodp;
        ...
        $a = pow(predict2::xke / $sat->deep_arg->xn, predict2::tothrd) * $tempa * $tempa;
        ...
        $sat->deep_arg->xn = predict2::xke / pow($a, 1.5);
        ...
        /* Solve Kepler's Equation */
        $capu = Predict2_Math::FMod2p ($xlt - $sat->deep_arg->xnode);
        $temp2 = $capu;
        ...
        /* Phase in rads */
        $sat->phase = $xlt - $sat->deep_arg->xnode - $sat->deep_arg->omgadf + predict2::twopi;
        if ($sat->phase < 0.0) {
            $sat->phase += predict2::twopi;
        }
        $sat->phase = Predict2_Math::FMod2p ($sat->phase);
    ...
    public function Deep($ientry, Predict2_Sat $sat)
    {
        switch ($ientry) {
        case self::dpinit : /* Entrance for deep space initialization */
            $sat->dps->thgr = Predict2_Time::ThetaG($sat->tle->epoch, $sat->deep_arg);
            $eq = $sat->tle->eo;
            ...
            if ($day != $sat->dps->preep) {
                ...
                $sat->dps->zmol = Predict2_Math::FMod2p($c - $gam);
                $zx = 0.39785416 * $stem / $sat->dps->zsinil;
                $zy = $sat->dps->zcoshl * $ctem + 0.91744867 * $sat->dps->zsinhl * $stem;
                $zx = Predict2_Math::AcTan($zx, $zy);
                $zx = $gam + $zx - $xnodce;
                $sat->dps->zcosgl = cos($zx);
                $sat->dps->zsingl = sin($zx);
                $sat->dps->zmos = 6.2565837 + 0.017201977 * $day;
                $sat->dps->zmos = Predict2_Math::FMod2p($sat->dps->zmos);
            } /* End if(day != preep) */

            /* Do solar terms */
            $sat->dps->savtsn = 1E20;
            $zcosg = predict2::zcosgs;
            $zsing = predict2::zsings;
            $zcosi = predict2::zcosis;
            $zsini = predict2::zsinis;
            $zcosh = $cosq;
            $zsinh = $sinq;
            $cc = predict2::c1ss;
            $zn = predict2::zns;
            ...
            if (!(($sat->dps->xnq < 0.0052359877) && ($sat->dps->xnq > 0.0034906585))) {
                ...
                $bfact = $sat->deep_arg->xmdot + $sat->deep_arg->xnodot +
                    $sat->deep_arg->xnodot - predict2::thdt - predict2::thdt;
                ...
                $sat->dps->del2 = 2 * $sat->dps->del1 * $f220 * $g200 * predict2::q22;
                $sat->dps->del3 = 3 * $sat->dps->del1 * $f330 * $g300 * predict2::q33 * $aqnv;
                $sat->dps->del1 = $sat->dps->del1 * $f311 * $g310 * predict2::q31 * $aqnv;
                ...
                $bfact = $sat->deep_arg->xmdot + $xpidot - predict2::thdt;
        ...
        case self::dpsec: /* Entrance for deep space secular effects */
            ...
            if ($sat->deep_arg->xinc < 0) {
                ...
                $sat->deep_arg->xnode = $sat->deep_arg->xnode + predict2::pi;
                $sat->deep_arg->omgadf = $sat->deep_arg->omgadf - predict2::pi;
            }
            ...
            do {
                ...
                do {
                    ...
                    if ($sat->flags & self::SYNCHRONOUS_FLAG) {
                        ...
                        $xndot = $sat->dps->d2201 * sin($x2omi + $sat->dps->xli - predict2::g22)
                            + $sat->dps->d2211 * sin($sat->dps->xli - predict2::g22)
                            + $sat->dps->d3210 * sin($xomi + $sat->dps->xli - predict2::g32)
                            + $sat->dps->d3222 * sin(-$xomi + $sat->dps->xli - predict2::g32)
                            + $sat->dps->d4410 * sin($x2omi + $x2li- predict2::g44)
                            + $sat->dps->d4422 * sin($x2li- predict2::g44)
                            + $sat->dps->d5220 * sin($xomi + $sat->dps->xli- predict2::g52)
                            + $sat->dps->d5232 * sin(-$xomi + $sat->dps->xli- predict2::g52)
                            + $sat->dps->d5421 * sin($xomi + $x2li - predict2::g54)
                            + $sat->dps->d5433 * sin(-$xomi + $x2li - predict2::g54);
                        $xnddt = $sat->dps->d2201 * cos($x2omi + $sat->dps->xli- predict2::g22)
                            + $sat->dps->d2211 * cos($sat->dps->xli - predict2::g22)
                            + $sat->dps->d3210 * cos($xomi + $sat->dps->xli - predict2::g32)
                            + $sat->dps->d3222 * cos(-$xomi + $sat->dps->xli - predict2::g32)
                            + $sat->dps->d5220 * cos($xomi + $sat->dps->xli - predict2::g52)
                            + $sat->dps->d5232 * cos(-$xomi + $sat->dps->xli - predict2::g52)
                            + 2 * ($sat->dps->d4410 * cos($x2omi + $x2li - predict2::g44)
                                + $sat->dps->d4422 * cos($x2li - predict2::g44)
                                + $sat->dps->d5421 * cos($xomi + $x2li - predict2::g54)
                                + $sat->dps->d5433 * cos(-$xomi + $x2li - predict2::g54));
            ...
            $temp = -$sat->deep_arg->xnode + $sat->dps->thgr + $sat->deep_arg->t * predict2::thdt;
        ...
        case self::dpper: /* Entrance for lunar-solar periodics */
            ...
            if (abs($sat->dps->savtsn - $sat->deep_arg->t) >= 30) {
                ...
                $zm = $sat->dps->zmos + predict2::zns * $sat->deep_arg->t;
                $zf = $zm + 2 * predict2::zes * sin($zm);
                $sinzf = sin($zf);
                ...
                $zm = $sat->dps->zmol + predict2::znl * $sat->deep_arg->t;
                $zf = $zm + 2 * predict2::zel * sin($zm);
                $sinzf = sin($zf);
                $f2 = 0.5 * $sinzf * $sinzf - 0.25;
                $f3 = -0.5 * $sinzf * cos($zf);
            ...
            if ($sat->dps->xqncl >= 0.2) {
                ...
                $alfdp = $alfdp + $dalf;
                $betdp = $betdp + $dbet;
                $sat->deep_arg->xnode = Predict2_Math::FMod2p($sat->deep_arg->xnode);
                ...
                $sat->deep_arg->xnode = Predict2_Math::AcTan($alfdp, $betdp);

                /* This is a patch to Lyddane modification */
                /* suggested by Rob Matson. */
                if(abs($xnoh - $sat->deep_arg->xnode) > predict2::pi) {
                    if ($sat->deep_arg->xnode < $xnoh) {
                        $sat->deep_arg->xnode += predict2::twopi;
                    } else {
                        $sat->deep_arg->xnode -= predict2::twopi;
                    }
                }
     ...
     * Singleton
     *
     * @param Predict2_Sat $sat The current satellite data instance
     *
     * @return Predict2_SGPSDP
     */
    public static function getInstance(Predict2_Sat $sat)
    {
